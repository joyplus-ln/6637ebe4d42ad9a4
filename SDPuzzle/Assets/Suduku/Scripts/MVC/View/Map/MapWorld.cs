...
public class MapWorld : MonoBehaviour {
	...
	public Dictionary<int, MapLocation> locationsList = new Dictionary<int, MapLocation>();
	//[HideInInspector]
...
//				target_level = ProfileAssistant.main.local_profile.current_level;
			...
			OnPositionChanged(map_location);
		}
	}
	...
	public MapLocation ShowPreviuosLocation(MapLocation mapLocation) {
		...
		if (locationsList.ContainsKey(index))
			...
			AddLocation (previuosLocation);
		}

		return previuosLocation;
	}

	public void OnPositionChanged(MapLocation currentLocation) {
		int p = IsVisible(currentLocation.previousLocationConnector);
		int n = IsVisible(currentLocation.nextLocationConnector);
		if (n != 0 && p != 0 && p == n) {
			RemoveLocation(currentLocation);
			Destroy(currentLocation.gameObject);
			return;
		}
		if (currentLocation.nextLocation == null) {
			if (n == 0) {
				currentLocation.nextLocation = ShowNextLocation(currentLocation);
			}
		}
		if (currentLocation.previousLocation == null) {
			if (p == 0) {
				currentLocation.previousLocation = ShowPreviuosLocation(currentLocation);
			}
		}
	}
}
