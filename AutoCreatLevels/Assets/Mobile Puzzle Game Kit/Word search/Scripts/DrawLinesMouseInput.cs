...
public class DrawLinesMouseInput : MonoBehaviour {
	...
	void Update () {
		//at the start of a mouse click
		if(Input.GetMouseButtonDown(0)){
			//get the closest character (closest to the mouse click)
		...
		//if we're dragging the mouse and we're drawing a line
		else if(Input.GetMouseButton(0) && drawing){
			//get the character cell closest to the current mouse drag position
			...
			if(endCharacterCell != null){
				...
				//set the line size using distance as the x value and linewidth or cellwidth as the y value
			...
			//stop drawing if we release the mouse button
			drawing = false;
	...
	characterCell closestCharacter(){
		...
		for(int i = 0; i < wordSearch.characters.Count; i++){
			if(Vector3.Distance(Input.mousePosition, wordSearch.characters[i].label.transform.position) < smallestDistance){
				//if this character is closer than all previous characters, this is the closest character
				smallestDistance = Vector3.Distance(Input.mousePosition, wordSearch.characters[i].label.transform.position);
				closestCharacter = wordSearch.characters[i];
			}
		}
		
		//make sure the mouse actually hits the closest character
		if(smallestDistance > wordSearch.width/2)
			return null;
